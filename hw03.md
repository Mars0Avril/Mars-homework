...
## 1)C程序：int8_t x = -0x1f; int y = x； x = 0xe1；y = 0x111111e1因为int8类型只有一个字节，共八位，而int类型为四个字节，共32位，十六进制四位一个字符表示。原因：当把单字节的补码转化为更高字节的数字时，要将额外的位数都填为原有数的最高有效位。-0x1f是负数，最高有效位是1。
## 2）证明：int表示的二进制数共有32位，则其中0-2^31-1拿来表示正数，2^31到2^32-1表示负数。X为负数，与x的绝对值相加的和应为2^32（即一个1跟32个0）。由于x的绝对值的“一的补”（即反码）实际上是“2^32-1”这个数的每一位与x的绝对值所表示的数的每一位都相反的结果，x的绝对值的int类型表示为0x0000001f，故其反码为0x111111e0，所以y前六位为1。又因为若要求x的补码，应在求得反码0x111111e0之后再加上1，才能使该补码与x的绝对值相加等于2^32。所以y = 0x111111e1。
